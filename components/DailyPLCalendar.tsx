      ...
      <CardContent>
        ...
        <div className="grid grid-cols-7 gap-2">
          ...
          {daysInMonth.map((day) => {
            ...
            let cellClass = "aspect-square flex flex-col items-center justify-center rounded-lg border text-xs transition-all p-1 shadow-sm";
            
            if (!hasData) {
               cellClass += " border-border/40 text-muted-foreground/50 bg-secondary/20";
            } else if (isProfit) {
               cellClass += " border-green-500 bg-green-500/10 text-green-700 dark:text-green-400 font-bold";
            } else {
               cellClass += " border-red-500 bg-red-500/10 text-red-700 dark:text-red-400 font-bold";
            }

            return (
              <div key={day.toString()} className={cellClass} title={hasData ? `${data.count} trades` : undefined}>
                <span className={`mb-1 text-[10px] ${hasData ? 'opacity-90' : 'opacity-50'}`}>{format(day, 'd')}</span>
                {hasData && (
